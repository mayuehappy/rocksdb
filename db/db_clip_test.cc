...
TEST_F(DBClipTest, Test2) {
  ...
  db_->ClipColumnFamily(db_->DefaultColumnFamily(), Key(2), Key(5));
  ReadOptions ropts;

  // ASSERT_OK(db_->Get(ropts, Key(1), &result));
  Status s = db_->Get(ropts, Key(1), &result);
  ASSERT_TRUE(s.IsNotFound());

  ASSERT_OK(db_->Get(ropts, Key(2), &result));
  ASSERT_OK(db_->Get(ropts, Key(3), &result));
  ASSERT_OK(db_->Get(ropts, Key(4), &result));
  ASSERT_OK(db_->Get(ropts, Key(5), &result));
}
}  // namespace ROCKSDB_NAMESPACE
