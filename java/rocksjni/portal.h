...
class ExportImportFilesMetaDataJni : public JavaClass {
   ...
   *
   * @param env A pointer to the Java environment
   * @param export_import_files_meta_data A Cpp export import files meta data object
   *
   * @return A reference to a Java org.rocksdb.ExportImportFilesMetaData object, or
   * nullptr if an an exception occurs
   */
  static jobject fromCppExportImportFilesMetaData(
      JNIEnv* env, ROCKSDB_NAMESPACE::ExportImportFilesMetaData* export_import_files_meta_data) {
    jclass jclazz = getJClass(env);
    if (jclazz == nullptr) {
      // exception occurred accessing class
      return nullptr;
    ...
    jbyteArray jdb_comparator_name = ROCKSDB_NAMESPACE::JniUtil::copyBytes(env, export_import_files_meta_data->db_comparator_name);
    if (jdb_comparator_name == nullptr) {
      // exception occurred creating java byte array
      return nullptr;
    }

    const jsize jlen = static_cast<jsize>(export_import_files_meta_data->files.size());
    jobjectArray jfiles =
