  ...
  public void exportColumnFamily() throws RocksDBException {
    try (final Options options = new Options().
      ...
      try (final RocksDB db = RocksDB.open(options,
        ...
        db.put("key".getBytes(), "value".getBytes());
        try (final Checkpoint checkpoint = Checkpoint.create(db)) {
          ExportImportFilesMetaData metadata1 = checkpoint.exportColumnFamily(db.getDefaultColumnFamily(), checkpointFolder.
              getRoot().getAbsolutePath() + "/export_column_family1");
          assertThat(metadata1.files().size()).isEqualTo(1);
          assertThat(metadata1.dbComparatorName()).isEqualTo("leveldb.BytewiseComparator".getBytes());
          db.put("key2".getBytes(), "value2".getBytes());
          ExportImportFilesMetaData metadata2 =  checkpoint.exportColumnFamily(db.getDefaultColumnFamily(), checkpointFolder.
              getRoot().getAbsolutePath() + "/export_column_family2");
          assertThat(metadata2.files().size()).isEqualTo(2);
          assertThat(metadata2.dbComparatorName()).isEqualTo("leveldb.BytewiseComparator".getBytes());
        }
      }
    }
  }
